...
def get_status(url, redirect, timeout,  output_dir):
    try:
        r = requests.get(url, timeout=timeout, allow_redirects=redirect, verify=False) 

        if output_dir:
            temp = url.replace("://", ".").replace("/","")+".txt"
            filename = os.path.join(output_dir, temp)
            try:
                with open(filename, "w", encoding="utf-8") as f:
                    f.write(r.text)
            except OSError as e:
                print(Fore.RED + f"[-] Failed to save response for {url}: {e}")
...
def httpq(dom_list, redirect, timeout, output_dir):
    if os.path.isfile(dom_list):
        ...
        for dom in domains:
            get_status(dom, redirect, timeout, output_dir)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-o","--output-dir",help="Directory to save HTTP responses",type=str,default=None)
    ...
    httpq(args.domains, args.redirect, timeout, args.output_dir)
